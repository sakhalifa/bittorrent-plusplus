...
typedef struct job {
	...
	char *(*function)(void *v_arg, struct file **files, int *nb_file,
	    struct peer *peer); /* function pointer          */
...
static void *thread_do(struct thread *thread_p) {
	...
	while (threads_keepalive) {
		...
		if (threads_keepalive) {
			...
			char *(*func_buff)(void *v_arg, struct file **files, int *nb_file,
			    struct peer *peer);
			arg_t *arg_buff;
			...
			if (job_p) {
				...
				arg_buff  = job_p->arg;
				func_buff(arg_buff->command_arg, arg_buff->files,
				    arg_buff->nb_file, arg_buff->peer);
				free(job_p);
			}
...
static struct job *jobqueue_pull(jobqueue *jobqueue_p) {
	...
	case 0: /* if no jobs in queue */ break;
